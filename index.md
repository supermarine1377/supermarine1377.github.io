...
### 株式会社Insight Edge （2025年6月から）

住友商事グループのDXを推進する技術専門会社にて提供しているシステムの保守運用に携わっています。

運用保守チームの第1号社員として、弊社の運用保守の課題を探索ならびに定義し、開発チームから運用保守を手離れさせるための仕組みづくり、運用保守の属人性の低減などの施策を提案および実装をしています。

- 各システムの運用保守の情報の文書化
- 各システムの運用保守の情報を集約したデータウェアハウス構築
- 各システムの問い合わせ業務におけるルール制定、属人化解消の推進
- システム固有の定型的な作業のrunbook（手順書）作成
- 会社全体の運用保守のKGI/KPI策定、ダッシュボード作成
- 障害対応、および障害対応のルール制定
- 外部委託先の活用

このように、各システムを俯瞰した運用保守の施策、システム固有の運用保守の施策、ひいては会社全体の運用保守の施策を行なっているのが特徴です。

概要の一部をテックブログに寄稿しておりますので、よければご覧ください。
